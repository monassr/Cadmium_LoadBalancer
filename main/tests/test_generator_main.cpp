/*
Test main file for the generator atomic model.
*/
...
struct test_generator_coupled : public Coupled {
    test_generator_coupled(const std::string& id) : Coupled(id) {
        addComponent<generator>("generator", 1);
    }
};
...
extern "C" {
	...
	#endif
	{
		...
		auto model = std::make_shared<test_generator_coupled>("test_generator");
			...
			rootCoordinator.setLogger<CSVLogger>("simulation_results/generator_output.csv", ";");
		#endif
